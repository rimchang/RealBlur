function [ new_leftimg_cell, new_rightimg_cell ] = processing_scene_warping_from_disk( leftimg_cell, rightimg_cell, source_homography, warping_params)

        
        homography_cell = cell(1, size(leftimg_cell, 2));
        for j=1:size(leftimg_cell,2)
            tform_mat = load(fullfile(source_homography, 'tform', sprintf('tform_%d.mat', j)));
            tform_mat = tform_mat.foo;
            homography_cell{1,j} = tform_mat;
        end
